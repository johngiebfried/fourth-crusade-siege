 ...
 * `wallRoll` were blanked and the roster still carried round one's fama, so
 * everyone arrived at the sack order marked "Outside the walls" and the fama
 * lost for sitting out or losing a ship was refunded.
 *
 * The distinction is not obvious from either call site, which is why it lives
 * here as two named functions rather than as object literals in a component:
 *
 * - **Per-round**: where a man is attacking, which ship he is on, how far up
 *   the stages he has climbed *this* round. Reset.
 * - **The siege's memory**: fama, and how far he ever got — `status` and the
 *   roll that got him there. Kept, because the sack order is the whole reason
 *   the siege is being played and it is built from exactly these.
 */

/** End of a round: bank the results and take the sitting-out penalty. */
export function carryRound(players) {
  return players.map((p) => ({
    ...p,
    fama: p.attackChoice === 'sit_out' ? Math.max(0, p.fama - 1) : p.fama,
    attackChoice: null,
  }))
}
